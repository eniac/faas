...
def set_static_parameters(parameters):
    ...
    new_params = [
            ...
            "tasks.msieve.sqrt.threads = %d" % int(cps*spm),
            ]
...
def stage_required(stage):
    ...
    required = (utils.checkpoint['stage'] < stage_index)
...
def do_linalg(parameters, sieve_result):
    ...
    if utils.checkpoint.get('trial_division') != None:
        result['duration'] = 0
...
def main():
    ...
    utils.update_checkpoint(params)
    ...
    print(polysel_result, sieve_result, linalg_result, sqrt_result)
